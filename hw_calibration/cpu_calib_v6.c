...
/* ONE big LDIR (2000 bytes) per batch. No inner C loop (v4's 64-byte ldir got hoisted out
 * of the for-loop and the 200x push/pop overhead dominated). A single big ldir sits directly
 * in the while body, runs once per while-iteration, and 2000*7 = 14000 cycles DOMINATE the
 * batch, so LDRR is a clean read of the block-copy cost/byte regardless of hoisting. Global
 * arrays give safe, linker-placed buffers; g_dst / 0xBC00 is the destination. */
u8 g_src[2100];
u8 g_dst[2100];
...
u16 m_ldir_ram(void) {
    ...
    while (frames < FRAMES) {
        __asm(" push xhl"); __asm(" push xde"); __asm(" push xbc");
        __asm(" ld xhl,_g_src");
        __asm(" ld xde,_g_dst");
        __asm(" ld bc,2000");
        __asm(" ldirb (xde+),(xhl+)");
        __asm(" pop xbc"); __asm(" pop xde"); __asm(" pop xhl");
...
void main(void) {
    ...
    PrintString(P, PAL, 1, 1,  "CPU CALIB v6 /60f");
