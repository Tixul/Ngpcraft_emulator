/* ============================================================================
 * NGPC CPU-SPEED CALIBRATION ROM  (v3, 2026-07-16)
 *
 * v2 CONFIRMED on silicon: fetch is wait-stated (BASE etc. match cart_wait=3),
 * cart DATA reads are NOT (CRND == RRND), MUL/DIV were under-costed (now fixed).
 * With a fully silicon-matched CPU model Cool Boarders STILL runs ~51fps in the
 * emulator vs 30fps on silicon -> the residual is NOT the CPU. The one thing the
 * calib never measured is VRAM WRITES: the game does a per-frame ldir into char
 * RAM, and the K2GE may throttle CPU VRAM access during the active drawing period.
 *
 * v3 ADDS **VWR** = a VRAM write (0xBE00) in the same harness as MEM (a RAM write
 * at 0x4200). On silicon, VWR << MEM would confirm VRAM writes are wait-stated
 * (and by how much); VWR == MEM would refute it and re-open the analysis. The
 * batch loop spans active + vblank lines, so VWR reflects the average VRAM-write
 * cost across a frame (active lines are ~76% of the frame).
 * ==========================================================================
...
MEASURE(m_mem,   *(volatile u8 *)0x4200 = (u8)v)   /* RAM write (fast baseline) */
MEASURE(m_vram,  *(volatile u8 *)0xBE00 = (u8)v)   /* VRAM write (throttled?) -> vs MEM */
...
void main(void) {
    ...
    PrintString(P, PAL, 1, 1,  "CPU CALIB v3 /60f");
    ...
    PrintString(P, PAL, 1, 10, "CSEQ :");
    PrintString(P, PAL, 1, 11, "CRND :");
    PrintString(P, PAL, 1, 12, "RRND :");
    PrintString(P, PAL, 1, 14, "RASV :");
    PrintString(P, PAL, 1, 16, "read hw vs emu");
    ...
    while (1) {
        ...
        PrintDecimal(P, PAL, 12, 10, m_cart_seq(), 5);
        PrintDecimal(P, PAL, 12, 11, m_cart_rnd(), 5);
        PrintDecimal(P, PAL, 12, 12, m_ram_rnd(),  5);
        PrintDecimal(P, PAL, 12, 14, rasv_max(),   3);
    }
}
