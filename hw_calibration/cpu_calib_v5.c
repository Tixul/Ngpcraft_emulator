...
/* One 64-byte LDIR. The COUNT is loaded from a volatile global (g_n) every iteration,
 * so the optimiser cannot prove the block is loop-invariant and CANNOT hoist it out of
 * the REPS loop (v4's bug: an all-literal ldir was hoisted, so it ran ~once/batch and
 * the measurement was meaningless). We PUSH/POP the three registers LDIR touches. */
volatile u16 g_n = 64;
...
void main(void) {
    ...
    PrintString(P, PAL, 1, 1,  "CPU CALIB v5 /60f");
